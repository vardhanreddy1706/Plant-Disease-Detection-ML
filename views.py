...
class_labels = [ 'Apple___Apple_scab', 'Apple___Black_rot','Apple___Cedar_apple_rust','Apple___healthy','cotton___bacterial_blight','cotton___curl_virus',
                 'cotton___fussarium_wilt',
                 'cotton___healthy','Tomato___Early_blight',
                 'Tomato___healthy', 'Tomato___Septoria_leaf_spot',
...
def DiseasePredictionAction(request):
    if request.method == 'POST' and request.FILES['t1']:
        ...
        filename = fs.save('PlantDiseaseApp/static/samples/test.png', myfile)
        image = cv2.imread('PlantDiseaseApp/static/samples/test.png')
        ...
        img = cv2.imread('PlantDiseaseApp/static/samples/test.png')
        img = cv2.resize(img, (700,400))
        
        
        if os.path.exists('PlantDiseaseApp/static/samples/test.png'):
            os.remove('PlantDiseaseApp/static/samples/test.png')
        cv2.putText(img, 'Disease Detected & Classified as : '+class_labels[predict], (10, 25),  cv2.FONT_HERSHEY_SIMPLEX,0.7, (255, 0, 0), 2)
        cv2.putText(img, 'Fertilizer Recommended is : '+fertilizers[predict], (10, 45),  cv2.FONT_HERSHEY_SIMPLEX,0.7, (0, 0, 255), 2)
        cv2.imshow('Disease Detected & Classified as : '+class_labels[predict], img)
        cv2.waitKey(0)
        if predict==0:
            return render(request, 'Apple_scab.html')
        elif predict ==1:
            return render(request, 'Apple___Black_rot.html', {})
        elif predict ==2:
            return render(request, 'Apple___Cedar_apple_rust.html', {})
        elif predict ==3:
            return render(request, 'healthy.html', {})
        elif predict ==4:
            return render(request, 'cotton___bacterial_blight.html', {})
        elif predict ==5:
            return render(request, 'cotton___curl_virus.html', {})
        elif predict ==6:
            return render(request, 'cotton___fussarium_wilt.html', {})
        elif predict ==7:
            return render(request, 'healthy.html', {})
        elif predict ==8:
            return render(request, 'Tomato___Early_blight.html', {})
        elif predict ==9:
            return render(request, 'healthy.html', {})
        elif predict ==10:
            return render(request, 'Tomato___Septoria_leaf_spot.html', {})
        elif predict ==11:
            return render(request, 'Tomato___Tomato_Yellow_Leaf_Curl_Virus.html', {})
...
def runCNN(request):
    if request.method == 'GET':
        ...
        if os.path.exists('model/model.json'):
            ...
            AlexNet_layer = AlexNet(x)
            x = Conv2D(32, (3, 3), activation='relu')(AlexNet_layer)
        ...
        conf_matrix, output = CNNtestPrediction("Deep CNN Plant Disease Classification",classifier,X_test,y_test)   
        context= {'data':output}
...
def Signup(request):
    if request.method == 'POST':
        ...
        output = "none"
        con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'PlantDisease',charset='utf8')
        with con:
            cur = con.cursor()
            ...
            for row in rows:
                if row[0] == username:
                    ...
                    break
        if output == "none":
            db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'PlantDisease',charset='utf8')
